...
const ViewUsersModal: React.FC<ViewUsersModalProps> = ({ isOpen, onClose, classTitle, classId }) => {
  // Temporary fallback if classId isn't passed in (can be removed later)
  if (classId === null) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{LABELS.classSchedules.modals.viewUsers.title}</DialogTitle>
        </DialogHeader>
          ...
          <div className="flex flex-col gap-4 pr-2">
            {enrolledClassMembers.length > 0 ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {LABELS.classSchedules.modals.viewUsers.noMembers}
              </div>
            )}
          </div>
        </div>
